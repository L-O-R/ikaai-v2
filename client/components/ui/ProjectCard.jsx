...
const ProjectCard = ({ project, bg_color }) => {
  ...
  const [hasRequestedStats, setHasRequestedStats] = useState(
    Boolean(project.statistics?.length),
  );
  ...
  return (
    ...
    >
      <div
        className={`${bg_color} text-on-surface p-4 rounded-2xl transition-all duration-300 flex items-center justify-between h-24 group-hover:bg-primary group-hover:text-on-primary`}
      >
