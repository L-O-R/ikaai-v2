...
const Dropdown = ({ label, items, isOpen, toggle, close, headerTheme, isActive }) => {
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            ...
            <div
                className={`absolute z-50 -left-6 top-full mt-1 w-48 bg-surface rounded-xl shadow-lg border border-border-neutral overflow-hidden transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] origin-top ${isOpen ? 'opacity-100 scale-y-100 pointer-events-auto' : 'opacity-0 scale-y-95 pointer-events-none'
                    }`}
            >
                {items.map((item) => {
