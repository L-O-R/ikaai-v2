...
const Header = () => {
  ...
  return (
    <header>
      <nav
        className="bg-white fixed top-0 left-0 w-full max-w-full px-4 md:px-6 z-50 transition-all duration-300 ease-in-out py-2 text-on-surface overflow-x-clip"
        id="main-nav"
      >
        <div className="flex justify-between items-center w-full max-w-container-max mx-auto relative z-10">
          ...
          {/* Desktop Navigation */}
          <div
            className="hidden lg:flex items-center gap-5 xl:gap-8 flex-wrap justify-end"
            ref={navRef}
          >
            {pathname !== "/" && (
              <Link
            ...
            {navLinks.map((link, index) => {
              ...
              return (
                <Link
                  key={link.label}
                  href={link.href}
                  className={`nav-link font-sans text-body-sm font-semibold uppercase border-b-2 py-1 ${
                    isActive
          ...
          >
            <div className="relative w-6 h-5">
              <span
                className={`absolute left-0 w-6 h-0.5 bg-primary rounded-full transition-all duration-500 ease-[cubic-bezier(0.68,-0.55,0.27,1.55)] origin-center ${
                  isMobileMenuOpen ? "top-2 rotate-45 w-6" : "top-0 rotate-0"
                }`}
              />
              <span
                className={`absolute left-1/2 -translate-x-1/2 w-4 h-0.5 bg-primary/80 rounded-full transition-all duration-400 ease-out ${
                  isMobileMenuOpen
                    ? "opacity-0 scale-x-0 w-0"
                    : "opacity-100 scale-x-100 top-2"
                }`}
              />
              <span
                className={`absolute left-0 w-6 h-0.5 bg-primary rounded-full transition-all duration-500 ease-[cubic-bezier(0.68,-0.55,0.27,1.55)] origin-center ${
                  isMobileMenuOpen ? "top-2 -rotate-45 w-6" : "top-4 rotate-0"
                }`}
              />
            </div>
          </button>
        ...
        >
          <div
            className={`absolute inset-0 bg-surface/95 backdrop-blur-md transition-transform duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] ${
                ...
                {navLinks.map((link, index) => {
                  ...
                  return (
                    <Link
                      key={link.label}
                      href={link.href}
                      className={`block py-4 font-display text-2xl transition-colors border-b border-border-neutral last:border-0 ${
                        isActive
