...
const JobApplicationForm = ({ jobSlug }) => {
  ...
  const validateForm = () => {
    ...
    const driveRegex =
      /^(https?:\/\/)?(www\.)?(drive|docs)\.google\.com\/(file\/d\/|document\/d\/|spreadsheets\/d\/|presentation\/d\/|open\?id=|uc\?id=)([a-zA-Z0-9_-]+)/;
    if (!driveRegex.test(formData.resume_drive_link.trim())) {
      errors.resume_drive_link =
  ...
  const handleSubmit = async (event) => {
    ...
    const lastSubmitAt =
      typeof window !== "undefined"
        ? Number(sessionStorage.getItem(lastSubmitKey) || 0)
        : 0;
    ...
    try {
      ...
      setStatus("success");
      setMessage(
        "Application submitted successfully! Our team will review your details.",
      );
      ...
      const resumeErr = Array.isArray(data?.resume_drive_link)
        ? data.resume_drive_link[0]
        : undefined;
  ...
  return (
    <form
      ...
      className="space-y-4 rounded-2xl border border-border-neutral bg-surface p-6"
    >
      {message ? (
        <div
          className={`rounded-xl border px-4 py-3 text-sm transition-all ${status === "error" ? "border-red-200 bg-red-50 text-red-700" : "border-primary/20 bg-primary/10 text-primary"}`}
        >
          {message}
        </div>
      ) : null}
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <label className="block text-sm text-text-secondary">
          ...
          <input
            ...
            disabled={isDisabled}
            required
            className="w-full rounded-xl border border-border-neutral bg-white px-3 py-2 disabled:bg-neutral-100 disabled:text-text-muted"
          />
        ...
        <label className="block text-sm text-text-secondary">
          ...
          <input
            ...
            disabled={isDisabled}
            required
            className={`w-full rounded-xl border bg-white px-3 py-2 disabled:bg-neutral-100 disabled:text-text-muted ${formErrors.phone ? "border-red-500" : "border-border-neutral"}`}
          />
          {formErrors.phone ? (
            <p className="mt-1 text-xs text-red-600 font-medium">
              {formErrors.phone}
        ...
        <label className="block text-sm text-text-secondary">
          <span className="mb-2 block font-medium text-on-surface">
            Current Company
          </span>
          <input
        ...
        <label className="block text-sm text-text-secondary">
          ...
          <input
            ...
            className="w-full rounded-xl border border-border-neutral bg-white px-3 py-2 disabled:bg-neutral-100 disabled:text-text-muted"
          />
          {getFieldError(formErrors, "portfolio_url") ? (
            <p className="mt-1 text-xs text-red-600 font-medium">
              {getFieldError(formErrors, "portfolio_url")}
            </p>
          ) : null}
      ...
      <label className="block text-sm text-text-secondary">
        ...
        <input
          ...
          disabled={isDisabled}
          required
          className={`w-full rounded-xl border bg-white px-3 py-2 disabled:bg-neutral-100 disabled:text-text-muted ${formErrors.resume_drive_link ? "border-red-500" : "border-border-neutral"}`}
        />
        {!isSuccess && (
          <span className="mt-1 block text-xs text-text-muted italic">
            ⚠️ Make sure Link Sharing settings are set to "Anyone with the link
            can view".
          </span>
        )}
        {formErrors.resume_drive_link ? (
          <p className="mt-1 text-xs text-red-600 font-medium">
            {formErrors.resume_drive_link}
          </p>
        ) : null}
      ...
      <label className="block text-sm text-text-secondary">
        ...
        <textarea
          ...
          className="w-full rounded-xl border border-border-neutral bg-white px-3 py-2 disabled:bg-neutral-100 disabled:text-text-muted"
        />
        {getFieldError(formErrors, "cover_letter") ? (
          <p className="mt-1 text-xs text-red-600 font-medium">
            {getFieldError(formErrors, "cover_letter")}
          </p>
        ) : null}
